...
vec3col::vec3col(uint8_t R, uint8_t G, uint8_t B) : r(R), g(G), b(B) {}
...
void CWindow::SetButtonCallFunc(void(*buttonCall)(CWindow*, int b)) {
	ButtonOut = buttonCall;
}
LRESULT CALLBACK CWindow::WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	case WM_COMMAND:
	{
		...
		if (ButtonOut != nullptr)
			ButtonOut(m_wWindow, m_wWindow->m_ButtonMessage);
	}
	break;
